...
[.lang-python]
NOTE: Python controller methods declare the Reactive Streams `Publisher` type: a method returning a `Publisher` streams the emitted items, and a method annotated with ann:core.async.annotation.SingleResult[] returns the single emitted item as the response.
